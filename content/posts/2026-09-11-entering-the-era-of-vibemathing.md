...
### September 23, 2026 — The long story of odd zeta values meets new proof claims

For an integer $s>1$, the value $\zeta(s)=\sum_{n=1}^{\infty}n^{-s}$ asks a deceptively simple question: can this infinite sum equal a fraction of integers? A new preprint about $\zeta(5)$ and another about $\zeta(7)$ make more sense against the much longer history below. **A preprint's claim, a formal proof of a stated theorem, and broad acceptance of a new mathematical result are different milestones.**

| Date | Milestone | What it establishes, or does not yet establish |
| --- | --- | --- |
| **1735** | Euler solved the Basel problem: $\zeta(2)=\pi^2/6$. He later obtained formulas for the other positive even arguments. | These exact evaluations do not supply comparable formulas or irrationality proofs for the odd values. ([History of Euler's result](https://www.ams.org/bull/2013-50-04/S0273-0979-2013-01423-X/S0273-0979-2013-01423-X.pdf).) |
| **1978** | Roger Apéry proved that $\zeta(3)$ is irrational. | This is an established theorem. Apéry's [published account](https://www.numdam.org/item/AST_1979__61__11_0/) appeared in 1979; [Beukers' 1979 proof](https://doi.org/10.1112/blms/11.3.268) gave another route. |
| **2000** | Tanguy Rivoal proved that infinitely many odd zeta values are irrational. | The theorem does not identify every individual odd value. ([Paper](https://arxiv.org/abs/math/0008051).) |
| **2001** | Wadim Zudilin proved that at least one of $\zeta(5)$, $\zeta(7)$, $\zeta(9)$, and $\zeta(11)$ is irrational. | “At least one” does **not** say which one, much less that both $\zeta(5)$ and $\zeta(7)$ are irrational. ([Paper](https://doi.org/10.1070/RM2001v056n04ABEH000427).) |
| **September 17, 2026** | Aabir Fauzan released a [preprint claiming $\zeta(5)$ is irrational](https://zenodo.org/records/22826419). | This is a new claim with an explicit argument, not an older established theorem. |
| **September 23, 2026** | Prince Anand released a [preprint claiming $\zeta(7)$ is irrational](https://zenodo.org/records/22920911). | Its record notes a mismatch in a stated bound. I have not found a completed, independently checked formal proof of this claim; it should not be announced here as a theorem. |
| **By September 26, 2026** | Public Lean projects for $\zeta(5)$ report proofs of an irrationality endpoint. | The [domino14 project](https://github.com/domino14/zeta5) states the theorem using Mathlib's `riemannZeta` and reports only Lean's standard axioms; the [mo271 project](https://github.com/mo271/Zeta5) reports a separate completed formalization. Their claims and builds deserve independent inspection, but they are more concrete evidence than a preprint alone. |

The idea behind Apéry's proof is worth pausing over. He constructed exceptionally accurate rational approximations to $\zeta(3)$ and controlled their denominators through a recurrence. After clearing denominators, the argument produces **nonzero linear forms with integer coefficients** in $1$ and $\zeta(3)$ whose absolute values tend to zero. If $\zeta(3)$ were rational, a fixed denominator would turn these into nonzero integers smaller than $1$, an impossibility. The difficulty lies in obtaining approximations that are simultaneously accurate enough and arithmetically controlled. ([Apéry's account](https://www.numdam.org/item/AST_1979__61__11_0/); [Beukers' proof](https://doi.org/10.1112/blms/11.3.268).)

Fauzan's proposed $\zeta(5)$ argument uses a related irrationality strategy in a different form: integer polynomials $Q_n$ of degree at most $37n$ that are positive at $\zeta(5)$ yet bounded by $e^{-c n^2}$ for some $c>0$. Were $\zeta(5)=a/b$, the nonzero value $b^{37n}Q_n(a/b)$ would be an integer while its bound tends to zero. The hard work is proving the positivity, integrality, and decay for the *actual* constructed polynomials. The public Lean projects report checking these dependencies, with different implementations and stated assumptions. That is the specific claim to examine, rather than treating the phrase “Lean verified” as a substitute for reading the theorem and its axiom list. ([Preprint](https://zenodo.org/records/22826419); [formalization and axiom statement](https://github.com/domino14/zeta5); [independent formalization report](https://github.com/mo271/Zeta5).)

### September 25, 2026 — Claude reaches nine loops in a scattering-amplitude calculation

Anthropic published physicist Matt von Hippel's account of a challenge in theoretical particle physics: compute the six-particle amplitude in **planar $\mathcal N=4$ super Yang–Mills theory at nine loops**. A loop is an order of complexity in a perturbative calculation; this highly symmetric theory is a testing ground for methods, not a direct prediction for an experiment. The previous published amplitude had reached eight loops. ([Anthropic's account, including Lance Dixon's validation](https://www.anthropic.com/research/yes-claude-can-do-nine-loops).)

| When | What happened |
| --- | --- |
| **2023** | Dixon and Andrew Liu obtained the eight-loop amplitude by first studying a related, easier form factor and using antipodal duality. ([Prior result](https://arxiv.org/abs/2308.08199).) |
| **August–September 2026** | Anthropic physicists Liam Fitzpatrick and Siddharth Mishra-Sharma gave Claude Fable 5.1 the nine-loop task in the Claude Science research environment, then largely let it continue working. Claude used both a **direct amplitude bootstrap** and an **indirect form-factor route**. ([Account](https://www.anthropic.com/research/yes-claude-can-do-nine-loops).) |
| **September 1** | Dixon says he received Claude's result for validation. He checked it mainly through the corresponding form factor. ([His account](https://www.anthropic.com/research/yes-claude-can-do-nine-loops).) |
| **September 25** | Anthropic published the story and linked the [released nine-loop data](https://smsharma.io/cosmic-nine-loops/). A group led by Song He had reached a concurrent result for the symbol with some AI assistance. ([Anthropic's account](https://www.anthropic.com/research/yes-claude-can-do-nine-loops).) |

**The scale is easy to misstate.** At nine loops, the mathematical *symbol* has weight $18$: an expanded term is an ordered word of 18 letters from a specified alphabet, with a coefficient. The released data say that, **on the parity-preserving surface $\Delta=0$**, this symbol would contain **30,024,320,034 nonzero weight-18 words if written out in full**, counted modulo one prime. The team did not print a 30-billion-term expression. It stored the result in much more compact coproduct form; the same restricted-surface data comprise 279 components with 295,186,924 terms. The 30-billion count refers to that restricted symbol, not to the unrestricted full amplitude or to 30 billion separate calculations. ([Data and term counts](https://smsharma.io/cosmic-nine-loops/).)

How did Claude get there? The physicists supplied the target, and Claude organized and coded an existing **bootstrap** strategy: start from a constrained space of possible answers, then use mathematical and physical consistency conditions to narrow it. It also followed the established form-factor/antipodal-duality route. The direct calculation used Python and SymPy; von Hippel estimates about **96 CPUs for a week** for its computational part, roughly **$100** of a total **$1,000–$2,000** per route for a paying user. The striking contribution was carrying a long, fragile, expert-built recipe through to a checkable result with limited human steering. Von Hippel does not claim it invented a new physical principle, and the concurrent human–AI computation matters when judging novelty and priority. ([Anthropic's account](https://www.anthropic.com/research/yes-claude-can-do-nine-loops); [released methods and validation record](https://smsharma.io/cosmic-nine-loops/).)
